...
function listenToDeals() {
    ...
    listenToEvent(globalContractInstance.Claim({
        _claimer: getPrivateAccount()
    }), "claim");
    listenToEvent(globalContractInstance.Deposit({
        _depositor: getPublicAccount()
    }), "deposit");
    listenToEvent(globalContractInstance.Withdraw({
        _withdrawer: getPrivateAccount()
    }), "withdraw");
    listenToEvent(globalContractInstance.Withdraw({
        _withdrawer: getPublicAccount()
    }), "withdraw");
}
...
function getDeals() {
    ...
    var getAllEvents = function(newEvent, eventType, finalCallback) {
        newEvent.get(function(err, logs) {
            ...
            newEvent.stopWatching(function(){});
            finalCallback();
        });
    };

    var getPrivateWithdrawFunc = function() {
        getAllEvents(globalContractInstance.Withdraw({
            _withdrawer: getPrivateAccount()
        }, {
            fromBlock: 0,
            toBlock: 'latest'
    ...
    var getPublicWithdrawFunc = function() {
        getAllEvents(globalContractInstance.Withdraw({
            ...
            fromBlock: 0,
            toBlock: 'latest'
        }), "withdraw", getPrivateWithdrawFunc);
    };
    var getDepositFunc = function() {
        getAllEvents(globalContractInstance.Deposit({
            _depositor: getPublicAccount()
        }, {
            fromBlock: 0,
            toBlock: 'latest'
        }), "deposit", getPublicWithdrawFunc);
    };
    var getClaimFunction = function() {
        getAllEvents(globalContractInstance.Claim({
            _claimer: getPrivateAccount()
        }, {
            fromBlock: 0,
            toBlock: 'latest'
        }), "claim", getDepositFunc);
    };
    var getNewDealsFunction = function() {
        getAllEvents(globalContractInstance.NewDeal({}, {
            fromBlock: 0,
            toBlock: 'latest'
        }), "newDeal", getClaimFunction);
    };
...
function listenToEnoughClaimsEvent(dealId) {
    var event = globalContractInstance.EnoughClaims({
        _dealId: dealId
    }, {
        fromBlock: 0,
        toBlock: 'latest'
    ...
    event.get(function(err, logs) {
        ...
        event.stopWatching(function(){});
        if (logSize > 0) return;
        var listenEvent = globalContractInstance.EnoughClaims({
            _dealId: dealId
        }, {
            fromBlock: 0,
            toBlock: 'latest'
        ...
        listenEvent.watch(function(err, result) {
            ...
            listenEvent.stopWatching(function(){}); // this is one time event
        });
...
function listenToFullyFundedEvent(dealId) {
    var event = globalContractInstance.DealFullyFunded({
        _dealId: dealId
    }, {
        fromBlock: 0,
        toBlock: 'latest'
    ...
    event.get(function(err, logs) {
        ...
        event.stopWatching(function(){});
        if (logSize > 0) return;
        var listenEvent = globalContractInstance.DealFullyFunded({
            _dealId: dealId
        }, {
            fromBlock: 0,
            toBlock: 'latest'
...
var makeClaimFunc = function(event) {
    ...
    globalContractInstance.makeClaim(dealId, {
            from: sender,
            value: dealDefaultParams._claimDepositInWei,
            gas: maxGas
        },
        function(err, result) {
            if (err) return handleError(err);
            dealUI.clickStart();
            waitForEventConifrmFunc(globalContractInstance.Claim({
                    _dealId: dealId,
                    _claimer: sender
                }),
                result,
...
var makeDepositFunc = function(event) {
    ...
    globalContractInstance.makeDeposit(dealId, {
            from: sender,
            value: dealDefaultParams._claimUnitValueInWei,
            gas: maxGas
        },
        function(err, result) {
            if (err) return handleError(err);
            dealUI.clickStart();
            waitForEventConifrmFunc(globalContractInstance.Deposit({
                    _dealId: dealId,
                    _depositor: sender
                }),
                result,
...
var makeWithdrawFunc = function(event) {
    ...
    globalContractInstance.withdraw(dealId, {
        ...
        function(err, result) {
            if (err) return handleError(err);
            dealUI.clickStart();
            waitForEventConifrmFunc(globalContractInstance.Withdraw({
                    _dealId: dealId,
                    _withdrawer: sender
                }),
                result,
